...
#Alpha Beta ============================================================
    ...
    def selectMove(self, representation, depth):
        ...
        for p in range(len(representation.my_pieces)):
            piece = representation.my_pieces[p]
            representation.generateMov(piece)
            moves = piece.listMov
            p1 = piece.position
            for p2 in moves:
    ...
    def alpha_beta(self, representation, depth, alpha, beta, maximizingPlayer):
        ...
        for col in range(8):
            ...
            if representation.getP((7,col)) == PAWN:
                return representation.my_color * 10000
            # Black victory
        ...
        if maximizingPlayer:
            ...
            for p in range(len(representation.op_pieces)):
       ...
       # myScore
        ...
        if self.my_color == WHITE:
            for piece in self.my_pieces:
                if piece.type == PAWN:
                    ...
                    myScore = myScore + V_PAWN*2^(row-1)*M_PAWN
                if piece.type == ROOK:
                    self.generateMov(piece)
                    nbsMov = len(piece.listMov)
            ...
            for piece in self.my_pieces:
                if piece.type == PAWN:
                    row = piece.position[0]
                    myScore = myScore + V_PAWN*2^(6-row)*M_PAWN
                if piece.type == ROOK:
                    self.generateMov(piece)
                    nbsMov = len(piece.listMov)
                    myScore = myScore + V_OTHER+nbsMov
        ...
        if self.my_color != WHITE:
            for piece in self.op_pieces:
                if piece.type == PAWN:
                    row = piece.position[0]
                    opScore = opScore + V_PAWN*2^(row-1)*M_PAWN
                if piece.type == ROOK:
                    self.generateMov(piece)
                    nbsMov = len(piece.listMov)
            ...
            for piece in self.op_pieces:
                if piece.type == PAWN:
                    row = piece.position[0]
                    opScore = opScore + V_PAWN*2^(6-row)*M_PAWN
                if piece.type == ROOK:
                    self.generateMov(piece)
                    nbsMov = len(piece.listMov)
                    opScore = opScore + V_OTHER+nbsMov
